...
const { chromium } = require("playwright"); // all playwright packages should be installed

describe("End to End tests", () => {
  let clientUrl = "http://127.0.0.1:5500/02.Book-Library/"; // change it with your own Live Server URL when testing
  let browser;
  let page;
  let mockBaseContent;
  ...
  it("loads books", async () => {
    ...
    await page.route("**/jsonstore/collections/books", (x) =>
      x.fulfill(fakeRes)
    );
  ...
  it("edits books", async () => {
    let bookList = {
      original: {
        id: "3",
        book: { author: "Gosho", title: "pesho plays a lot" },
      },
